...
ggplot() +
#   geom_sf(data = river_expansion, fill = "orange", colour = "orange") +    # Uncomment me once you've built your extra polygons - GL doesn't need one.
...
#### Save average flow rate for N concentration ####

daily <- rivers[coords,][,                                                 # Summarise pixels in the model domain
                           .(Volume = weighted.mean(Runoff, Area, na.rm = T)), # By totaling Kg of water by pixel area 
                           by = .(Month, Year)] %>%                         # By time step
  mutate(Runoff = Volume/1000) %>%                                          # scaled to daily from per seconds, and Kg to m^3 
  dplyr::select(-Volume) %>% 
  drop_na()                                                                 # Drop pixels which didn't contribute water for some time steps

# Way too small!
#saveRDS(daily, "./Objects/Daily river input.rds")


summary2 <- rivers[coords,][,                                                # Summarise pixels in the model domain
                           .(Volume = sum(Runoff*Area, na.rm = T)),                                  # By totaling Kg of water by pixel area 
                           by = .(Month, Year)] %>%                                                  # By time step
  mutate(Runoff = Volume/1000) %>%                                  # scaled to daily from per seconds, and Kg to m^3 
  dplyr::select(-Volume) %>% 
  drop_na()                                                                 # Drop pixels which didn't contribute water for some time steps
#About 10x bigger than the maximum in the observations.

write_rds(summary2, "./Objects/daily freshwater total.rds") 
