...
TS <- readRDS("./Objects/TS.rds") %>%                                       # Read in time series
  mutate(across(c(Ice_pres, Ice_conc_avg, Ice_Thickness_avg, Snow_Thickness_avg), 
                ~ ifelse(is.na(.x) & Compartment != "Offshore D", 0, .x)))  # Replace Ice related NAs with 0s for plotting
