...
S <- nrow(nc_lat)*(5/6) ; W <- length(nc_lon)*100/360 ; E <- length(nc_lon)*1/2     # For Mercatore
...
#add_overlay(overlay = overlay, alphalayer = 0.1, alphamethod = 'multiply') %>% 
  ...
  plot_3d(mat, zscale = 100, fov = 0, theta = -120, phi = 30, 
...
tic()   
render_camera(theta=-120,phi=30,fov=60,zoom= 0.3)                                       # Change camera view after initial plotting 
toc()
