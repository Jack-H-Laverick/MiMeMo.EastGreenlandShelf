...
S <- nrow(nc_lat)*(5.3/6) ; W <- length(nc_lon)*100/360 ; E <- length(nc_lon)*1/2     # For Mercatore
...
mat <- matrix[seq(nrow(matrix), 1, by = -4),]                               # Use for full resolution, divide zscales by 10                
...
mat %>%
  ...
  add_shadow(ambient) %>%
  plot_3d(mat, zscale = 20, fov = 60, theta = -120, phi = 30, 
