...
#output <- readRDS("./Objects/MMM_GEBCO_grid.rds")                          # Load in Bathymetry pulled from GEBCO
...
#  slice(c(1:144)) %>%
  ...
  future_map(Big_Currents)                                                  # Perform the extraction and save an object for each month (in parallel)
toc()                                                                       # Stop timing 
