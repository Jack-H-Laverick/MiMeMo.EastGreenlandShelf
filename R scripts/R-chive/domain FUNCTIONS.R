...
get_weights <- function(top, bottom)           {
  ...
  weights <- array(NA, c(490,264,27))                                          # Initialise an array to hold weights
  first <- weights[,,1]
...
get_weights.pred <- function(top, bottom)      {
  ...
  weights <- array(NA, c(490,264,38))                                            # Initialise an array to hold weights
  first <- weights[,,1]
  ...
  weights[,,38] <- mask_bathy - mean(Space.pred$nc_depth[37:38])                 # The remaining water column thickness is the sea floor - the deepest midpoint. 
...
get_sea   <- function(file)                    {
  ...
  output %>%                                                                   # Grab the tidied dataframe of lat-longs
    ...
    st_join(domains) %>%                                                       # Find out which points are in the domains
...
get_sal.pred <- function(file)                 {
  ...
  nc_raw <- nc_open(file)                                                    # Open up a netcdf file to see it's raw contents (var names)
  nc_saline <- ncvar_get(nc_raw, "vosaline", start3DP, count3DP)             # Pull salinity
  nc_close(nc_raw)                                                           # You must close an open netcdf file when finished to avoid data loss
  output %>%                                                                 # Grab the tidied dataframe of lat-longs
...
get_temp.pred <- function(file)                {
  ...
  nc_raw <- nc_open(file)                                                    # Open up a netcdf file to see it's raw contents (var names)
  nc_temp <- ncvar_get(nc_raw, "votemper", start3DP, count3DP)               # Pull temperature
  nc_close(nc_raw)                                                           # You must close an open netcdf file when finished to avoid data loss
  output %>%                                                                 # Grab the tidied dataframe of lat-longs
...
get_merid.pred <- function(file)               {
  ...
  nc_raw <- nc_open(file)                                                    # Open up a netcdf file to see it's raw contents (var names)
  nc_merid <- ncvar_get(nc_raw, "vomecrty", start3DP, count3DP)              # Pull meridinal currents
  nc_close(nc_raw)                                                           # You must close an open netcdf file when finished to avoid data loss
  output %>%                                                                 # Grab the tidied dataframe of lat-longs
...
get_zonal.pred <- function(file)               {
  ...
  nc_raw <- nc_open(file)                                                    # Open up a netcdf file to see it's raw contents (var names)
  nc_zonal <- ncvar_get(nc_raw, "vozocrtx", start3DP, count3DP)              # Pull zonal current
  nc_close(nc_raw)                                                           # You must close an open netcdf file when finished to avoid data loss
  output %>%                                                                 # Grab the tidied dataframe of lat-longs
...
summarise_months <- function(month, get, type) {
  ...
  if(type == "S") {
    Monthly <- month %>%                                                     # Take the year
      ...
      group_by(Longitude, Latitude, Region, Shore, Year, Month, Bathymetry,Shore_dist) %>% # Group by pixel and time step
...
str_sub(data$Year, -1, -1) <- "0"                       # Overwite the 4th digit with a 0 to get the decade
  ...
  decades <- group_by(data, Longitude, Latitude, Year, Month, Shore, Region, Bathymetry,Shore_dist) %>%  # Group by pixel and decade
    rename(decade = Year) %>%
...
combine        <- function(files)              {
  ...
  combined <- files %>% 
    ...
    decadal() %>%
    saveRDS(., file = paste("~/Data/NEMO - MEDUSA/domain.sp",type, "rds", sep = ".")) # save out the data object
  # return(combined)
...
Compartmentalise <- function(work)             {
  ...
  inshore <- data.frame(work[["Inshore"]]) %>%                             # Maniuplate just Inshore observations 
    filter(Depth == "Shallow") %>%                                         # There is no deep compartment inshore
  ...
  offshore$weights[offshore$weights > 60 & offshore$Depth == "Shallow" ] <- 60 # Observation in water deeper than 60 m with a shallow label should have a water thickness of 60 m
  offshore$weights[offshore$Depth == "Deep" ] <- offshore$weights[offshore$Depth == "Deep" ] - 60 # All deep water observations have a water thickness of the bathymetry - the shallow layer thickness
  
  result <- rbind(inshore, offshore)
  return(result)
}    # Refilter and get weights for depths and shore zones

get_sea_ts   <- function(file)                 {
  ...
  nc_mixed <- ncvar_get(nc_raw, "somxl010", start2D, count2D)                  # Extract a matrix of mixed layer depths
  nc_close(nc_raw)                                                             # You must close an open netcdf file when finished to avoid data loss
  work <- output %>%                                                           # Grab the tidied dataframe of lat-longs
    mutate(S_Ice = as.numeric(nc_ice),                                         # Append new variable to coordinates (no depths for ice)
           S_Turbocline = as.numeric(nc_turbocline),                           # Append new variable to coordinates (no third dimension for turbocline)
           S_Mixed = as.numeric(nc_mixed),                                     # Append new variable to coordinates (no third dimension for mixed layer depth)
           S_Salinity = as.numeric(stratify(nc_saline, Shallow_mark, sw)),     # Collapse shallow salinity into 2D and convert to long format
           S_Temperature = as.numeric(stratify(nc_temp, Shallow_mark, sw)),    # Collapse shallow temperatures into 2D and convert to long format
           D_Salinity = as.numeric(stratify(nc_saline, Deep_mark, dw)),
           D_Temperature = as.numeric(stratify(nc_temp, Deep_mark, dw))) %>%   # Collapse, reshape and append deepwater data
    ...
    gather(key = Depth, value = "Value", S_Ice:D_Temperature) %>%            # Introduce a long format depth column
    mutate(Variable = str_sub(Depth, start = 3),                             # Duplicate the column of variable names, one to give us depths, the other to reintroduce column names 
           Depth = str_sub(Depth, 1,1)) %>%                                  # NAs will appear unless each variable has the same depth coding
    spread(key = Variable, value = Value) %>%                                # Respread to get a column for each variable 
    filter(Shore_dist > 0) %>%                                               # Remove points on land
    mutate(Depth = as.factor(Depth)) %>%                                     # Convert depth to a factor for plotting
    Compartmentalise()
  return(work)                                                         # Drop points outside of the polygons
  # Drop points outside of the polygons
  
}    # Pull ice, turbocline, mixed layer depth, from S files and then salinity and temperature  in three depth bands
get_bio_ts   <- function(file)                 { 
  print(str_glue("getting Dissolved Inorganic Nitrogen and Chlorophyll data from {file}"))
  nc_raw <- nc_open(file)                                                    # Open up a netcdf file to see it's raw contents (var names)
  nc_DIN <- ncvar_get(nc_raw, "DIN", start3D, count3D)                                         # Extract a matrix for the variable
  nc_CHD <- ncvar_get(nc_raw, "CHD", start3D, count3D)
  ...
  work <- output %>%                                                         # Grab the tidied dataframe of lat-longs
    ...
    gather(key = Depth, value = "Value", S_DIN:D_Chlorophyll) %>%            # Introduce a long format depth column
    mutate(Variable = str_sub(Depth, start = 3),                             # Duplicate the column of variable names, one to give us depths, the other to reintroduce column names 
           Depth = str_sub(Depth, 1,1)) %>%                                  # NAs will appear unless each variable has the same depth coding
    spread(key = Variable, value = Value) %>%                                # Respread to get a column for each variable 
    filter(Shore_dist > 0) %>%                                               # Remove points on land
...
get_merid_ts <- function(file)                 {
  ...
  work <- output %>%                                                         # Grab the tidied dataframe of lat-longs
    ...
    st_join(domains) %>%                                                     # Find out which points are in the domains
    drop_na(Region)%>%                                                       # Drop points outside of the polygons
    gather(key = Depth, value = "Meridional", S_Meridional, D_Meridional) %>%# Introduce a long format depth column
    filter(Shore_dist > 0) %>%                                               # Remove points on land
    mutate(Depth = as.factor(Depth)) %>%                                     # Convert depth to a factor for plotting
    Compartmentalise()
  return(work)                                                         # Drop points outside of the polygons
...
get_zonal_ts <- function(file)                 {
  ...
  work <- output %>%                                                         # Grab the tidied dataframe of lat-longs
    ...
    st_join(domains) %>%                                                     # Find out which points are in the domains
    drop_na(Region) %>%                                                      # Drop points outside of the polygons
    gather(key = Depth, value = "Zonal", S_Zonal, D_Zonal) %>%               # Introduce a long format depth column
    filter(Shore_dist > 0) %>%                                               # Remove points on land
    mutate(Depth = as.factor(Depth)) %>%                                     # Convert depth to a factor for plotting
    Compartmentalise()
  return(work)                                                         # Drop points outside of the polygons
...
get_sal.pred_ts <- function(file)              {
  ...
  work <- output %>%                                                         # Grab the tidied dataframe of lat-longs
    ...
    st_join(domains) %>%                                                     # Find out which points are in the domains
    drop_na(Region) %>%                                                      # Drop points outside of the polygons
    gather(key = Depth, value = "Salinity", S_Salinity, D_Salinity) %>%      # Introduce a long format depth column
    filter(Shore_dist > 0) %>%                                               # Remove points on land
    mutate(Depth = as.factor(Depth)) %>%                                     # Convert depth to a factor for plotting
    Compartmentalise()
  return(work)                                                         # Drop points outside of the polygons
...
get_temp.pred_ts <- function(file)             {
  print(str_glue("getting Temperature predictions from {file}"))
  nc_raw <- nc_open(file)                                                    # Open up a netcdf file to see it's raw contents (var names)
  nc_temp <- ncvar_get(nc_raw, "votemper", start3DP, count3DP)               # Pull temperature
  nc_close(nc_raw)                                                           # You must close an open netcdf file when finished to avoid data loss
  work <- output %>%                                                         # Grab the tidied dataframe of lat-longs
    ...
    st_join(domains) %>%                                                     # Find out which points are in the domains
    drop_na(Region)%>%                                                       # Drop points outside of the polygons
    gather(key = Depth, value = "Temperature", S_Temperature, D_Temperature) %>%   # Introduce a long format depth column
    filter(Shore_dist > 0) %>%                                               # Remove points on land
    mutate(Depth = as.factor(Depth)) %>%                                     # Convert depth to a factor for plotting
    Compartmentalise()
  return(work)                                                         # Drop points outside of the polygons
...
get_merid.pred_ts <- function(file)            {
  print(str_glue("getting Meridional current predictions from {file}"))
  nc_raw <- nc_open(file)                                                    # Open up a netcdf file to see it's raw contents (var names)
  nc_merid <- ncvar_get(nc_raw, "vomecrty", start3DP, count3DP)              # Pull meridinal currents
  nc_close(nc_raw)                                                           # You must close an open netcdf file when finished to avoid data loss
...
get_zonal.pred_ts <- function(file)            {
  print(str_glue("getting Zonal current predictions from {file}"))
  nc_raw <- nc_open(file)                                                    # Open up a netcdf file to see it's raw contents (var names)
  nc_zonal <- ncvar_get(nc_raw, "vozocrtx", start3DP, count3DP)              # Pull zonal current
  nc_close(nc_raw)                                                           # You must close an open netcdf file when finished to avoid data loss
...
summarise_months_ts <- function(month, get, type) {
  ...
  if(type == "S")                {
    Monthly <- month %>%                                                     # Take the year
      ...
      summarise(Ice = mean(Ice, na.rm = TRUE),                               # Get monthly mean sea ice             
                Turbocline = mean(Turbocline, na.rm = TRUE),                 # Get monthly minimum turbocline depth
                Mixed = mean(Mixed, na.rm = TRUE),                           # Get monthly minimum mixed layer depth
                Salinity = weighted.mean(Salinity, weights, na.rm = TRUE),   # Get monthly mean salinity
                Temperature = weighted.mean(Temperature, weights, na.rm = TRUE)) # Calculate monthly average at each pixel
    return(Monthly) }                  # You will need to mutate different columns depending on
  if(type == "P")                {                                             
    Monthly <- month %>%                                                     # Take the year
      mutate(data = purrr::map(value, get)) %>%                              # Extract data from each file
      unnest(data) %>%                                                       # Extract all encoded data
      group_by(Region, Shore, Year, Month, Depth) %>%
      summarise(DIN = weighted.mean(DIN, weights, na.rm = TRUE),  
                Chlorophyll = weighted.mean(Chlorophyll, weights, na.rm = TRUE))  # Calculate monthly average for each zone
    return(Monthly) }                  # the extraction function used
  if(type %in% c("V", "V.pred")) {                                                          # the extraction function used
    Monthly <- month %>%                                                     # Take the year
      mutate(data = purrr::map(value, get)) %>%                              # Extract data from each file
      unnest(data) %>%                                                       # Extract all encoded data
      group_by(Region, Shore, Year, Month, Depth) %>%
      summarise(Meridional = weighted.mean(abs(Meridional), weights, na.rm = TRUE)) # Calculate average absolute current speed in each zone
    return(Monthly) }
  if(type %in% c("U", "U.pred")) {                                           # the extraction function used
    Monthly <- month %>%                                                     # Take the year
      mutate(data = purrr::map(value, get)) %>%                              # Extract data from each file
      unnest(data) %>%                                                       # Extract all encoded data
      group_by(Region, Shore, Year, Month, Depth) %>%
      summarise(Zonal = weighted.mean(abs(Zonal), weights, na.rm = TRUE))    # Calculate monthly average at each compartment
    return(Monthly)}                                        
  if(type == "S.pred")           {                                                     # the extraction function used
    Monthly <- month %>%                                                     # Take the year
      mutate(data = purrr::map(value, get)) %>%                              # Extract data from each file
      unnest(data) %>%                                                       # Extract all encoded data
      group_by(Region, Shore, Year, Month, Depth) %>%
      summarise(Salinity = weighted.mean(Salinity, weights, na.rm = TRUE))   # Calculate monthly average per zone
    return(Monthly)}                                                  
  if(type == "T.pred")           {                                                     # the extraction function used
    Monthly <- month %>%                                                     # Take the year
      mutate(data = purrr::map(value, get)) %>%                              # Extract data from each file
      unnest(data) %>%                                                       # Extract all encoded data
      group_by(Region, Shore, Year, Month, Depth) %>%
      summarise(Temperature = weighted.mean(Temperature, weights, na.rm = TRUE))      # Calculate monthly average at each pixel
    return(Monthly)}                                                  
  
} # Extract the data from files in a month and average by zone
time_series <- function(files)                 {

  type <- files[1,5]                                                         # Pull type from the file
  
  if(type == "S") get <- get_sea_ts                                          # Change the extracting function based on file contents
  if(type == "P") get <- get_bio_ts
  if(type == "V") get <- get_merid_ts
  if(type == "U") get <- get_zonal_ts
  
  if(type == "S.pred") get <- get_sal.pred_ts                                # Change the extracting function based on file contents
  if(type == "T.pred") get <- get_temp.pred_ts                               # Change the extracting function based on file contents
  if(type == "V.pred") get <- get_merid.pred_ts
  if(type == "U.pred") get <- get_zonal.pred_ts
  
  combined <- files %>% 
    split(., f = list(.$Year, .$Month)) %>%                                  # Create a list of dataframes for each separate month 
    .[sapply(., function(x) dim(x)[1]) > 0] %>%                              # Drop empty dataframes (Months which weren't observed but split introduces)
    lapply(summarise_months_ts, get, type) %>%                                  # Perform the extraction
    rbindlist() %>%                                                          # Collapse the list into a dataframe
    saveRDS(., file = paste("~/Data/NEMO - MEDUSA/domain.ts",type, "rds", sep = ".")) # save out the data object
   return(combined)
...
longify <- function(data, strip)               {
  ...
  work <- select(data, c(decade, Month, Region, Shore, x, y, Bathymetry, Shore_dist, strip)) %>% # Pull columns of interest from main data file
    ...
    mutate(depth = as.factor(depth),                                        # Convert depth to a factor for plotting
           ...
           Month = as.factor(Month))                                        # Change  integers to factor
...
split_decade <- function(data)                 { 
  split(data, f = data$decade) %>% 
    .[sapply(., function(x) dim(x)[1]) > 0] }    # Simplify the splitting lines for use with lapply
split_shore <- function(data)                  { 
  split(data, f = data$Shore) %>% 
    .[sapply(., function(x) dim(x)[1]) > 0] }    # Simplify the splitting lines for use with lapply
...
point_plot <- function(data)                   {
  ...
  if (type == "Ice")    {
    base <- base +
      ...
      coord_sf(xlim = c(6100000, 2800000), ylim = c(7500000, 4400000)) +
      NULL
    ggsave(paste("FIG_domain_", type, " ", depth, " ", decade, ".png", sep = ""), 
...
ts_plot <- function(var)                       {
  ...
  ts <- ggplot(TS) +
    geom_line(aes(x=date, y= get(var), colour = Compartment)) +
    facet_grid(Region ~.) +
    theme_minimal() +
    labs(title = "Historical time series by compartment", y = var) +
    NULL
  ggsave(paste("FIG_domain_TS_", var, ".png"), plot = ts, width = 16, height = 10, units = "cm", dpi = 500)
...
stick_plot <- function(data, direction)        {
  ...
  ggsave(paste("FIG_domain_Currents", unique(data$depth), unique(data$decade), ".png"), plot = sticks, scale = 1, width = 16, height = 10, units = "cm", dpi = 500)
  # return(sticks)
